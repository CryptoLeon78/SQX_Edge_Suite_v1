...
Comandos:
  list                                    Lista los minings del plan configurado
  generate --mining N [--template PATH]   Genera un .cfx para el mining N
  generate-all [--template PATH]          Genera todos los .cfx del plan
...
def cmd_generate(args) -> int:
    ...
    out_path = generate_project(
        ...
        sqx_db_path=cfg.get("sqx_data_db"), broker_postfix=cfg.get("darwinex_suffix") or DEFAULT_BROKER_POSTFIX,
...
def main(argv=None) -> int:
    ...
    sub.add_parser("list", help="List configured minings")

    g = sub.add_parser("generate", help="Generate one .cfx for a mining")
    g.add_argument("--mining", type=int, required=True, help="Mining number from config/plan.json")
    ...
    a = sub.add_parser("generate-all", help="Generate .cfx for every configured mining")
