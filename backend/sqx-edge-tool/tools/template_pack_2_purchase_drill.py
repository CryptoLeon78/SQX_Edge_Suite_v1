...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "template_pack_2_purchase_drill.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "template_pack_2_purchase_drill"
...
def redact_email(value: str) -> str:
    ...
    redacted_local = f"{local[0]}***{local[-1]}" if len(local) > 2 else f"{local[0]}***"
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    if not publication_path.is_file():
        findings.append("template_pack_2_publication_config_missing")
    else:
        publication = load_json(publication_path)
        if publication.get("state") != depends_on.get("publicationState"):
            findings.append("template_pack_2_publication_state_invalid")
...
def validate_manifest_variant(config: dict[str, Any], manifest: dict[str, Any], provider_variant_id: str) -> list[str]:
    findings: list[str] = []
    variant = checkout_variant(manifest, str(config.get("offerId", "template_pack_2")))
    if variant is None:
        return ["template_pack_2_variant_missing"]
    ...
    if variant.get("billing") != config.get("billing"):
        findings.append("template_pack_2_billing_mismatch")
...
def validate_delivery_package(path_value: str) -> list[str]:
    ...
    required_entries = (
        "pro-template-pack-2/README.md",
        "pro-template-pack-2/presets/strategy_import_template_pack_2.csv",
        "pro-template-pack-2/checklists/delivery_checklist.md",
        "pro-template-pack-2/checklists/support_boundaries.md",
        "pro-template-pack-2/checklists/safe_claims.md",
    )
...
def decision_from(
    ...
    blockers.extend(f"{key}_not_confirmed" for key, value in confirmations.items() if not value)
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Template Pack 2 Controlled Purchase Drill Evidence",
        "",
    ...
    lines.extend(f"- {name}: `{value}`" for name, value in report["confirmations"].items())
    lines.append("")
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"template_pack_2_purchase_drill_{current_stamp}.json"
    md_path = output_dir / f"template_pack_2_purchase_drill_{current_stamp}.md"
...
def collect_purchase_drill(
    ...
    report: dict[str, Any] = {
        "created_at": now_iso(),
        "state": config.get("state", "template_pack_2_purchase_drill_ready"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate a controlled purchase drill for Template Pack 2.")
