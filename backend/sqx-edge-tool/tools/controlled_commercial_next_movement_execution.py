...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "controlled_commercial_next_movement_execution.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "controlled_commercial_next_movement_execution"
DEFAULT_DECISION_DIR = TOOL_ROOT / "data" / "controlled_commercial_next_movement"
...
def latest_decision_file(directory: Path = DEFAULT_DECISION_DIR) -> Path | None:
    return latest_file(directory, "controlled_commercial_next_movement_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    config_path = project_path(str(depends_on.get("controlledCommercialNextMovementConfig", "")))
    if not config_path.is_file():
        return ["controlled_commercial_next_movement_config_missing"]
    source_config = load_json(config_path)
    if source_config.get("state") != depends_on.get("controlledCommercialNextMovementState"):
        return ["controlled_commercial_next_movement_state_invalid"]
    return []


def validate_source(decision_record: dict[str, Any] | None, allow_no_go_decision: bool) -> list[str]:
    if decision_record is None:
        return ["controlled_commercial_next_movement_evidence_missing"]
    decision = decision_record.get("decision", {})
    if not decision.get("go") and not allow_no_go_decision:
        return ["controlled_commercial_next_movement_not_go", *decision.get("blockers", [])]
    return []
...
def execution_checklist() -> list[str]:
    return [
        "Load the approved M87 next-movement evidence before recording execution.",
        "Record only the exact M87-approved manual movement.",
...
def execution_decision_from(
    ...
    if source_movement not in set(config.get("allowedSourceNextMovements", [])):
        blockers.append("controlled_commercial_next_movement_execution_source_invalid")
    if execution_result not in set(config.get("allowedExecutionResults", [])):
        blockers.append("controlled_commercial_next_movement_execution_result_invalid")
    if decision_record and source_movement != source_next_movement(decision_record):
        blockers.append("controlled_commercial_next_movement_execution_source_not_m87_movement")
    if expected_result and execution_result != expected_result:
        blockers.append("controlled_commercial_next_movement_execution_result_not_allowed_for_m87_movement")
    if any(value < 0 for value in (new_private_links, new_invites)):
        blockers.append("controlled_commercial_next_movement_execution_metrics_invalid")
    if not safe_text(owner):
        blockers.append("controlled_commercial_next_movement_execution_owner_missing_or_unsafe")
    if not execution_summary.strip():
        blockers.append("controlled_commercial_next_movement_execution_summary_missing")
    if not next_monitor.strip():
        blockers.append("controlled_commercial_next_movement_execution_next_monitor_missing")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Controlled Commercial Next Movement Execution Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"controlled_commercial_next_movement_execution_{current_stamp}.json"
    md_path = output_dir / f"controlled_commercial_next_movement_execution_{current_stamp}.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Record the exact manual execution approved by the M87 next-movement gate.")
