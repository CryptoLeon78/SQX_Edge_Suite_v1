...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "next_controlled_commercial_movement_decision.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "next_controlled_commercial_movement_decision"
DEFAULT_MONITOR_DIR = TOOL_ROOT / "data" / "controlled_commercial_next_movement_execution_monitor"
EXPECTED_STATE = "next_controlled_commercial_movement_decision_ready"
...
def latest_monitor_file(directory: Path = DEFAULT_MONITOR_DIR) -> Path | None:
    return latest_file(directory, "controlled_commercial_next_movement_execution_monitor_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    monitor_config_path = project_path(str(depends_on.get("controlledCommercialNextMovementExecutionMonitorConfig", "")))
    if not monitor_config_path.is_file():
        return [*blockers, "controlled_commercial_next_movement_execution_monitor_config_missing"]
    monitor_config = load_json(monitor_config_path)
    if monitor_config.get("state") != depends_on.get("controlledCommercialNextMovementExecutionMonitorState"):
...
def validate_monitor(monitor: dict[str, Any] | None, allow_no_go_monitor: bool) -> list[str]:
    if monitor is None:
        return ["controlled_commercial_next_movement_execution_monitor_evidence_missing"]
    decision = monitor.get("decision", {})
    if not decision.get("go") and not allow_no_go_monitor:
        return ["controlled_commercial_next_movement_execution_monitor_not_go", *decision.get("blockers", [])]
    return []
...
def decision_checklist() -> list[str]:
    return [
        "Load the M89 monitor evidence before deciding any next commercial movement.",
...
def decision_from(
    ...
    if source_decision not in set(config.get("allowedSourceMonitorDecisions", [])):
        blockers.append("next_controlled_commercial_movement_source_decision_invalid")
    if next_movement not in set(config.get("allowedNextMovements", [])):
        blockers.append("next_controlled_commercial_movement_invalid")
    if monitor and source_decision != source_monitor_decision:
        blockers.append("next_controlled_commercial_movement_source_not_m89_decision")
    if any(value < 0 for value in (observation_hours, positive_signals, support_items_open, refund_requests, claims_issues, incidents)):
        blockers.append("next_controlled_commercial_movement_metrics_invalid")
    if not safe_text(owner):
        blockers.append("next_controlled_commercial_movement_owner_missing_or_unsafe")
    if not rationale.strip():
        blockers.append("next_controlled_commercial_movement_rationale_missing")
    if not next_gate.strip():
        blockers.append("next_controlled_commercial_movement_next_gate_missing")
    if observation_hours < int(config.get("minimumObservationHours", 24)):
        blockers.append("next_controlled_commercial_movement_requires_observation_time")
    ...
    for name, (value, maximum) in risk_counts.items():
        if value > maximum:
            blockers.append(f"next_controlled_commercial_movement_{name}_requires_hold_or_pause")

    if next_movement == "prepare_next_micro_step":
        if source_decision != "prepare_next_controlled_movement":
            blockers.append("prepare_next_micro_step_requires_m89_prepare_next_controlled_movement")
    ...
    if next_movement == "prepare_private_review_packet":
        if source_decision not in {"prepare_next_controlled_movement", "prepare_private_review_packet"}:
            blockers.append("prepare_private_review_packet_requires_m89_review_or_movement_decision")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Next Controlled Commercial Movement Decision Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"next_controlled_commercial_movement_decision_{current_stamp}.json"
    md_path = output_dir / f"next_controlled_commercial_movement_decision_{current_stamp}.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide the next controlled commercial movement from M89 evidence.")
