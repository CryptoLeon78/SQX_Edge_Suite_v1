...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "template_pack_1_publication.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "template_pack_1_publication"
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    offer_path = project_path(str(depends_on.get("offerConfig", "")))
    expected_state = depends_on.get("offerState")
    if not offer_path.is_file():
        return ["template_pack_1_offer_config_missing"]
    offer = load_json(offer_path)
    if offer.get("state") != expected_state:
        return ["template_pack_1_offer_state_invalid"]
    return []
...
def validate_values(config: dict[str, Any], manifest: dict[str, Any], values: dict[str, str]) -> list[str]:
    ...
    variant = variant_for(manifest, str(config.get("offerId", "template_pack_1")))
    if variant is None:
...
def apply_publication_values(
    ...
    offer_id = str(config.get("offerId", "template_pack_1"))
    live_status = str(config.get("liveStatusWhenApplied", "template_pack_1_controlled_publication_live"))

    checkout["status"] = live_status
    checkout["supportEmail"] = values["support_email"]
    checkout["templatePack1LiveCheckout"] = {
        "status": live_status,
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Template Pack 1 Live Checkout Publication Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"template_pack_1_publication_{current_stamp}.json"
    md_path = output_dir / f"template_pack_1_publication_{current_stamp}.md"
...
def collect_publication(
    ...
    report: dict[str, Any] = {
        ...
        "state": config.get("state", "template_pack_1_live_checkout_gate_ready"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate and optionally apply Template Pack 1 live checkout values.")
    ...
    parser.add_argument("--confirm-offer-reviewed", action="store_true")
    ...
    confirmations = {
        "offer_reviewed": args.confirm_offer_reviewed,
