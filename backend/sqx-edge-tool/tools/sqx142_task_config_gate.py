...
WFM_PERIOD_KEY = MC_PERIOD_KEY
WFM_DATA_TEST_PRECISION = MC_DATA_TEST_PRECISION
WFM_DATA_SESSION = MC_DATA_SESSION
WFM_DEFAULT_CHART_TARGET = MC2_DEFAULT_CHART_TARGET
WFM_DATA_ENGINE = SEQUENTIAL_DATA_ENGINE
WFM_CUSTOM_DATA_ENGINE = SEQUENTIAL_CUSTOM_DATA_ENGINE
...
def apply_synthetic_rankings_to_root(root: ET.Element, actions: list[dict[str, Any]]) -> None:
    apply_mc_rankings_to_root(
        root,
        actions,
        conditions_note="Synthetic pass/fail is owned by MonteCarloRetest/SyntheticBootstrapV3 acceptance conditions",
    )


def apply_synthetic_custom_data_static_to_root(root: ET.Element, actions: list[dict[str, Any]]) -> None:
    apply_synthetic_setup_to_root(root, "CustomData", SYNTHETIC_CUSTOM_DATA_ENGINE, actions)


def apply_synthetic_static_tabs_to_root(root: ET.Element) -> list[dict[str, Any]]:
    actions: list[dict[str, Any]] = []
    ...
    actions.append({"field": "Notes", "changed": False, "sha256": section_sha256(root, "Notes"), "note": "audited and preserved"})
    apply_mc_selected_strategies_to_root(root, actions)
    apply_synthetic_custom_data_static_to_root(root, actions)
    return actions


def synthetic_static_tabs_summary(root: ET.Element) -> dict[str, Any]:
    summary = mc_static_tabs_summary(root)
    custom = find_section(root, "CustomData")
    setup = custom.find(".//Setup") if custom is not None else None
    size_based = setup.find("./Commissions/Method[@type='SizeBased']/Params/Param[@key='Commission']") if setup is not None else None
...
def enforce_synthetic_static_tabs_guard(root: ET.Element) -> list[str]:
    issues: list[str] = []
    summary = synthetic_static_tabs_summary(root)
    ranking = summary.get("rankings") or {}
    if ranking.get("type") != "never":
        issues.append(f"Synthetic/Syntetic Rankings type is {ranking.get('type')!r}, expected 'never'")
    for key in ("MaxStrategies", "ConditionsType", "DeleteFailedStrategies", "ForceRunCrossChecks"):
        if ranking.get(key) != SYNTHETIC_RANKING_TARGET[key]:
            issues.append(f"Synthetic/Syntetic Rankings {key} is {ranking.get(key)!r}, expected {SYNTHETIC_RANKING_TARGET[key]!r}")
    if (ranking.get("FitPortfolio") or {}).get("active") != "false":
        issues.append("Synthetic/Syntetic FitPortfolio must remain disabled; portfolio selection belongs to later portfolio phases")
    if (ranking.get("CustomAnalysis") or {}).get("filter") != "false":
        issues.append("Synthetic/Syntetic CustomAnalysis filter must remain disabled")
    if ranking.get("conditions"):
        issues.append("Synthetic/Syntetic Rankings must not add extra conditions; SyntheticBootstrapV3 acceptance owns pass/fail")

    rmm = summary.get("riskMoneyManagement") or {}
    methods = rmm.get("methods") or {}
    for method_type, wanted in RETEST1_RISK_MONEY_MANAGEMENT_METHOD_TARGET.items():
        if methods.get(method_type) != wanted:
            issues.append(f"Synthetic/Syntetic RiskMoneyManagement {method_type} is {methods.get(method_type)!r}, expected {wanted!r}")

    atms = summary.get("atms") or {}
    for key, wanted in RETEST1_ATMS_TARGET.items():
        if (atms.get("attrs") or {}).get(key) != wanted:
            issues.append(f"Synthetic/Syntetic ATMs {key} is {(atms.get('attrs') or {}).get(key)!r}, expected {wanted!r}")

    selected = summary.get("selectedStrategies") or {}
    if selected.get("children") != 0 or selected.get("text"):
        issues.append("Synthetic/Syntetic SelectedStrategies must remain empty in the base template")

    custom = summary.get("customData") or {}
    if not custom.get("exists"):
        issues.append("Synthetic/Syntetic CustomData section missing")
    else:
        period = generator_period(SYNTHETIC_PERIOD_KEY)
        setup = custom.get("setup") or {}
        chart = custom.get("chart") or {}
        if setup.get("dateFrom") != period[0] or setup.get("dateTo") != period[1]:
            issues.append(f"Synthetic/Syntetic CustomData dates are {(setup.get('dateFrom'), setup.get('dateTo'))!r}, expected {period!r}")
        if setup.get("testPrecision") != SYNTHETIC_DATA_TEST_PRECISION:
            issues.append(f"Synthetic/Syntetic CustomData testPrecision is {setup.get('testPrecision')!r}, expected {SYNTHETIC_DATA_TEST_PRECISION!r}")
        if setup.get("session") != SYNTHETIC_DATA_SESSION:
            issues.append(f"Synthetic/Syntetic CustomData session is {setup.get('session')!r}, expected {SYNTHETIC_DATA_SESSION!r}")
        if chart != SYNTHETIC_DEFAULT_CHART_TARGET:
            issues.append(f"Synthetic/Syntetic CustomData chart seed is {chart!r}, expected {SYNTHETIC_DEFAULT_CHART_TARGET!r}")
        if custom.get("commission") != MC_CUSTOM_DATA_COMMISSION_TARGET:
            issues.append(f"Synthetic/Syntetic CustomData commission is {custom.get('commission')!r}, expected {MC_CUSTOM_DATA_COMMISSION_TARGET!r}")
        if custom.get("mainTestValues") != SYNTHETIC_CUSTOM_DATA_MAIN_TEST_VALUES_TARGET:
            issues.append("Synthetic/Syntetic CustomData MainTestValues drifted from Synthetic dual-carrier target")
    ...
    for token in MC_BANNED_DONOR_TOKENS:
        if token in guarded_text:
            issues.append(f"Forbidden donor token leaked into Synthetic/Syntetic static tabs: {token}")
    if re.search(r"[A-Za-z]:\\", guarded_text):
        issues.append("Local absolute path leaked into Synthetic/Syntetic static tabs")

    for issue in enforce_synthetic_passive_generation_guard(root):
        issues.append(f"Passive generation guard: {issue}")
    return issues


def update_synthetic_static_tabs_target_in_cfx(cfx: Path, backup_root: Path, apply: bool) -> dict[str, Any]:
    payload: dict[str, Any] = {
        "path": str(cfx),
        "exists": cfx.is_file(),
        "isZip": bool(cfx.is_file() and zipfile.is_zipfile(cfx)),
    ...
    before_text = serialize_xml(root)
    payload["before"] = synthetic_static_tabs_summary(root)
    payload["actions"] = apply_synthetic_static_tabs_to_root(root)
    payload["after"] = synthetic_static_tabs_summary(root)
    payload["issues"] = enforce_synthetic_static_tabs_guard(root)
    payload["guardOk"] = not payload["issues"]
    after_text = serialize_xml(root)
    payload["changed"] = before_text != after_text
    payload["changedActionCount"] = sum(1 for item in payload["actions"] if item.get("changed"))
    payload["targetValues"] = {
        "rankings": SYNTHETIC_RANKING_TARGET,
        "rankingConditions": [],
        "riskMoneyManagementMethods": RETEST1_RISK_MONEY_MANAGEMENT_METHOD_TARGET,
        "atms": RETEST1_ATMS_TARGET,
        "staticTabs": SYNTHETIC_STATIC_TABS,
        "customDataMainTestValues": SYNTHETIC_CUSTOM_DATA_MAIN_TEST_VALUES_TARGET,
        "customDataCommission": MC_CUSTOM_DATA_COMMISSION_TARGET,
        "customDataCarrier": "dual_synced",
    }
    payload["targetRationale"] = {
        "decision": "Synthetic static tabs close the last inert surfaces before phase closeout while keeping SyntheticBootstrapV3 as the only active robustness decision.",
        "ranking": "Synthetic pass/fail is owned by MonteCarloRetest/SyntheticBootstrapV3 acceptance filters; Ranking must preserve failed rows and not run portfolio selection.",
        "riskMoneyManagement": "FixedSize keeps Capa1 retests comparable and avoids sizing noise.",
        "customData": "Synthetic keeps SQX142-compatible Data+CustomData dual carrier synchronized; this block hardens only the CustomData tab without deleting Data.",
        "staticTabs": "ATMs, Notes and SelectedStrategies stay inert while executable behavior is guarded by previous Synthetic blocks.",
    }
    if apply and payload["changed"] and payload["guardOk"]:
        backup = backup_file(cfx, backup_root)
        payload["backup"] = str(backup)
...
def promote_synthetic_static_tabs_target(root142: Path, project_root: Path, target: str, apply: bool) -> dict[str, Any]:
    ensure_ledger(project_root)
    backup_root = ledger_root(project_root) / "backups" / f"phase10_synthetic_static_tabs_{stamp()}"
    targets: dict[str, Path] = {}
    if target in {"local-base", "both"}:
        targets["localBase"] = cfx_for_project(root142, DEFAULT_BASE_PROJECT)
    if target in {"repo-template", "both"}:
        targets["repoTemplate"] = DEFAULT_TEMPLATE
    results = {
        name: update_synthetic_static_tabs_target_in_cfx(path, backup_root / name, apply=apply)
        for name, path in targets.items()
    }
    payload: dict[str, Any] = {
        "ok": all(
        ...
        "createdAt": now_iso(),
        "phase": "phase10",
        "operation": "synthetic_static_tabs_target",
        "apply": apply,
        "target": target,
        "results": results,
        "nextPhase": "phase10_synthetic_static_tabs_diff_review" if not apply else SYNTHETIC_STATIC_TABS_NEXT,
    }
    evidence_target = ledger_root(project_root) / "diffs" / f"phase10_synthetic_static_tabs_target_{stamp()}.json"
    write_json(evidence_target, payload)
    payload["written"] = str(evidence_target)
    return payload
...
def wfm_data_databanks_resources_options_summary(root: ET.Element | None) -> dict[str, Any]:
    ...
    if data_setup is not None and custom_setup is not None:
        setup_pairs.append({
            ...
            "customData": {
                ...
                "slippage": custom_setup.get("slippage", ""),
                "minDist": custom_setup.get("minDist", ""),
                "engine": custom_setup.get("engine", ""),
                "chart": dict(custom_setup.find("Chart").attrib) if custom_setup.find("Chart") is not None else {},
            },
        })
    summary["carrierDecision"] = {
        "mode": "dual_synced",
        "reason": "WFM in SQX142 stores both Data and CustomData; keep both for compatibility but enforce the same period, precision, session and chart seed. Engine remains section-specific.",
        "pairs": setup_pairs,
    }
    return summary


def apply_wfm_setup_to_root(root: ET.Element, section_name: str, engine: str, actions: list[dict[str, Any]]) -> ET.Element:
    section = find_section(root, section_name)
    if section is None:
        section = ET.SubElement(root, section_name)
        actions.append({"field": section_name, "from": None, "to": "created", "changed": True})
    ...
    if chart is None:
        chart = ET.SubElement(setup, "Chart")
        actions.append({"field": f"{section_name}/Setup/Chart", "from": None, "to": dict(chart.attrib), "changed": True})
    set_attrs_on_node(chart, WFM_DEFAULT_CHART_TARGET, actions, f"{section_name}/Setup/Chart:attrs")
    ensure_commission_method(setup, actions, f"{section_name}/Setup")
    if section_name == "CustomData":
        main_values = setup.find("MainTestValues")
        if main_values is None:
        ...
        set_attrs_on_node(
            main_values,
            WFM_CUSTOM_DATA_MAIN_TEST_VALUES_TARGET,
            actions,
            "CustomData/MainTestValues:attrs",
        )
    return setup
...
def apply_wfm_data_databanks_resources_options_to_root(root: ET.Element) -> list[dict[str, Any]]:
    actions: list[dict[str, Any]] = []
    data_setup = apply_wfm_setup_to_root(root, "Data", WFM_DATA_ENGINE, actions)
    custom_setup = apply_wfm_setup_to_root(root, "CustomData", WFM_CUSTOM_DATA_ENGINE, actions)
    ...
    actions.append({
        ...
        "to": [],
        "changed": bool(removed_oos),
        "note": "WFM consumes SPP survivors and does not add a nested OOS split.",
    })
    ...
    actions.append({
        ...
        "to": "dual_synced",
        "changed": False,
        "note": "Kept both Data and CustomData for SQX142 compatibility; enforced matching period, precision, session and chart seed while preserving section-specific engines.",
    })
    return actions


def enforce_wfm_data_databanks_resources_options_guard(root: ET.Element) -> list[str]:
    issues: list[str] = []
    period = generator_period(WFM_PERIOD_KEY)
    data_setup = root.find("./Data/Setups/Setup")
    custom_setup = root.find("./CustomData/Setups/Setup")
    if data_setup is None:
        issues.append("WFM Data/Setup missing")
    if custom_setup is None:
        issues.append("WFM CustomData/Setup missing")
    for label, setup, engine in (("Data", data_setup, WFM_DATA_ENGINE), ("CustomData", custom_setup, WFM_CUSTOM_DATA_ENGINE)):
        if setup is None:
            continue
        if setup.get("dateFrom") != period[0] or setup.get("dateTo") != period[1]:
            issues.append(f"WFM {label} dates are not {WFM_PERIOD_KEY}")
        if setup.get("testPrecision") != WFM_DATA_TEST_PRECISION:
            issues.append(f"WFM {label} testPrecision must stay {WFM_DATA_TEST_PRECISION}")
        if setup.get("session") != WFM_DATA_SESSION:
            issues.append(f"WFM {label} session must stay {WFM_DATA_SESSION}")
        if setup.get("engine") != engine:
            issues.append(f"WFM {label} engine is {setup.get('engine')!r}, expected {engine!r}")
        chart = setup.find("Chart")
        if chart is None:
            issues.append(f"WFM {label} chart missing")
        else:
            for key, wanted in WFM_DEFAULT_CHART_TARGET.items():
                if chart.get(key) != wanted:
                    issues.append(f"WFM {label} chart {key} is {chart.get(key)!r}, expected {wanted!r}")
        commission = setup.find("./Commissions/Method[@type='SizeBased']/Params/Param[@key='Commission']")
        if (commission.text if commission is not None else "") != MC_CUSTOM_DATA_COMMISSION_TARGET:
            issues.append(f"WFM {label} commission is {(commission.text if commission is not None else '')!r}, expected {MC_CUSTOM_DATA_COMMISSION_TARGET!r}")
    if data_setup is not None and custom_setup is not None:
        data_chart = data_setup.find("Chart")
        custom_chart = custom_setup.find("Chart")
        for key in ("dateFrom", "dateTo", "testPrecision", "session", "slippage", "minDist"):
            if data_setup.get(key) != custom_setup.get(key):
                issues.append(f"WFM Data/CustomData setup mismatch for {key}")
        if data_chart is not None and custom_chart is not None:
            for key in ("symbol", "timeframe", "spread"):
                if data_chart.get(key) != custom_chart.get(key):
                    issues.append(f"WFM Data/CustomData chart mismatch for {key}")
    main_values = root.find("./CustomData/Setups/Setup/MainTestValues")
    if main_values is None or dict(main_values.attrib) != WFM_CUSTOM_DATA_MAIN_TEST_VALUES_TARGET:
    ...
    resources = find_section(root, "Resources")
    if resources is None:
        issues.append("WFM Resources missing")
    else:
        chart_symbols = {
            chart.get("symbol", "")
            for chart in root.findall("./CustomData/Setups/Setup/Chart")
        ...
        if chart_symbols != resource_symbols:
            issues.append(f"WFM custom chart/resource mismatch: charts={sorted(chart_symbols)} resources={sorted(resource_symbols)}")
        broker_ids = {
            broker.get("id", "")
            for broker in resources.findall("./Brokers/Broker")
            if broker.get("id")
        ...
        for symbol in resources.findall("./Symbols/Symbol"):
            if symbol.get("precision") != MC_RESOURCE_PRECISION:
                issues.append(f"WFM resource {symbol.get('name')} precision is not TICK")
            if symbol.get("timezone") != MC_RESOURCE_TIMEZONE:
                issues.append(f"WFM resource {symbol.get('name')} timezone is not EETUS")
            if symbol.get("broker") not in broker_ids:
                issues.append(f"WFM resource {symbol.get('name')} references missing broker {symbol.get('broker')}")
            info = symbol.find("InstrumentInfo")
            if info is None:
                issues.append(f"WFM resource {symbol.get('name')} has no nested InstrumentInfo")
            elif info.get("broker") not in broker_ids:
                issues.append(f"WFM nested InstrumentInfo for {symbol.get('name')} references missing broker {info.get('broker')}")
        if resources.findall("./Sessions/Session"):
            issues.append("WFM resources must not keep session entries")
    ...
    for key, wanted in WFM_OPTIONS_PARAMS_TARGET.items():
        if params.get(key) != wanted:
            issues.append(f"WFM Options param {key} is {params.get(key)!r}, expected {wanted!r}")

    if root.findall("./Data/OutOfSample/Range"):
        issues.append("WFM Data must not contain nested OOS ranges")
    guarded_text = (
        section_text(root, "Data")
        + section_text(root, "CustomData")
        + section_text(root, "Databanks")
    ...
    for token in MC_BANNED_DONOR_TOKENS:
        if token in guarded_text:
            issues.append(f"Forbidden donor token leaked into WFM Data/Databanks/Resources/Options: {token}")
    if re.search(r"[A-Za-z]:\\", guarded_text):
        issues.append("Local absolute path leaked into WFM Data/Databanks/Resources/Options")
    return issues


def update_wfm_data_databanks_resources_options_target_in_cfx(cfx: Path, backup_root: Path, apply: bool) -> dict[str, Any]:
    payload: dict[str, Any] = {
        "path": str(cfx),
        "exists": cfx.is_file(),
        "isZip": bool(cfx.is_file() and zipfile.is_zipfile(cfx)),
    ...
    before_text = serialize_xml(root)
    payload["before"] = wfm_data_databanks_resources_options_summary(root)
    payload["actions"] = apply_wfm_data_databanks_resources_options_to_root(root)
    payload["after"] = wfm_data_databanks_resources_options_summary(root)
    payload["issues"] = enforce_wfm_data_databanks_resources_options_guard(root)
    payload["guardOk"] = not payload["issues"]
    after_text = serialize_xml(root)
    payload["changedActionCount"] = sum(1 for item in payload["actions"] if item.get("changed"))
    payload["xmlChanged"] = before_text != after_text
    ...
    payload["targetValues"] = {
        "taskTitle": WFM_TASK_TITLE,
        "taskXml": WFM_TASK_XML,
        "periodKey": WFM_PERIOD_KEY,
        "dateFrom": generator_period(WFM_PERIOD_KEY)[0],
        "dateTo": generator_period(WFM_PERIOD_KEY)[1],
        "dataCarrier": "dual_synced",
        "dataEngine": WFM_DATA_ENGINE,
        "customDataEngine": WFM_CUSTOM_DATA_ENGINE,
        "customDataMainTestValues": WFM_CUSTOM_DATA_MAIN_TEST_VALUES_TARGET,
        "databanks": WFM_EXPECTED_DATABANKS,
        "resourcePrecision": MC_RESOURCE_PRECISION,
        "resourceTimezone": MC_RESOURCE_TIMEZONE,
        "options": WFM_OPTIONS_PARAMS_TARGET,
        "executionPolicy": WFM_EXECUTION_POLICY,
    }
    payload["targetRationale"] = {
        "methodology": "WFM consumes SPP survivors and is kept review-only because SPP has not been approved/executed as a live gate.",
        "carrier": "SQX142 WFM carries both Data and CustomData; keeping both synced is safer than deleting either source without UI evidence.",
        "engines": "Data remains MetaTrader5 (hedged) and CustomData remains MetaTrader4, matching existing SQX142 carrier roles while synchronizing dates, precision, session and chart seed.",
        "options": "Trading time ranges are disabled for this review gate; Project Generator must not inject timeframe windows into AutomaticRetest-Task4.xml.",
        "naturalResults": "The block preserves natural passed/failed rows and does not force Results=passed.",
    }
    if apply and payload["changed"] and payload["guardOk"]:
        backup = backup_file(cfx, backup_root)
...
def promote_wfm_data_databanks_resources_options_target(root142: Path, project_root: Path, target: str, apply: bool) -> dict[str, Any]:
    ensure_ledger(project_root)
    backup_root = ledger_root(project_root) / "backups" / f"phase12_wfm_data_databanks_resources_options_{stamp()}"
    targets: dict[str, Path] = {}
    if target in {"local-base", "both"}:
        targets["localBase"] = cfx_for_project(root142, DEFAULT_BASE_PROJECT)
    if target in {"repo-template", "both"}:
        targets["repoTemplate"] = DEFAULT_TEMPLATE
    results = {
        name: update_wfm_data_databanks_resources_options_target_in_cfx(path, backup_root / name, apply=apply)
        for name, path in targets.items()
    }
    payload: dict[str, Any] = {
        "ok": all(
        ...
        "createdAt": now_iso(),
        "phase": "phase12",
        "operation": "wfm_data_databanks_resources_options_target",
        "apply": apply,
        "target": target,
        "results": results,
        "nextPhase": "phase12_wfm_data_databanks_resources_options_diff_review" if not apply else WFM_DATA_DATABANKS_RESOURCES_OPTIONS_NEXT,
    }
    evidence_target = ledger_root(project_root) / "diffs" / f"phase12_wfm_data_databanks_resources_options_target_{stamp()}.json"
    write_json(evidence_target, payload)
    payload["written"] = str(evidence_target)
    return payload
...
def build_parser() -> argparse.ArgumentParser:
    ...
    synthetic_static = sub.add_parser("synthetic-static-tabs-target")
    ...
    wfm_data = sub.add_parser("wfm-data-databanks-resources-options-target")
