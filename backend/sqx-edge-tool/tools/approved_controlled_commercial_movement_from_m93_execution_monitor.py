...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "approved_controlled_commercial_movement_from_m93_execution_monitor.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "approved_controlled_commercial_movement_from_m93_execution_monitor"
DEFAULT_EXECUTION_DIR = TOOL_ROOT / "data" / "approved_controlled_commercial_movement_from_m93_execution"
EXPECTED_STATE = "approved_controlled_commercial_movement_from_m93_execution_monitor_ready"
...
def latest_execution_file(directory: Path = DEFAULT_EXECUTION_DIR) -> Path | None:
    return latest_file(directory, "approved_controlled_commercial_movement_from_m93_execution_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    if config.get("state") != EXPECTED_STATE:
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_state_invalid")
    depends_on = config.get("dependsOn") if isinstance(config.get("dependsOn"), dict) else {}
    execution_config_path = project_path(str(depends_on.get("approvedControlledCommercialMovementFromM93ExecutionConfig", "")))
    if not execution_config_path.is_file():
        return [*blockers, "approved_controlled_commercial_movement_from_m93_execution_config_missing"]
    execution_config = load_json(execution_config_path)
    if execution_config.get("state") != depends_on.get("approvedControlledCommercialMovementFromM93ExecutionState"):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_state_invalid")
    return blockers


def validate_execution(execution: dict[str, Any] | None, allow_no_go_execution: bool) -> list[str]:
    if execution is None:
        return ["approved_controlled_commercial_movement_from_m93_execution_evidence_missing"]
    decision = execution.get("decision", {})
    if not decision.get("go") and not allow_no_go_execution:
        return ["approved_controlled_commercial_movement_from_m93_execution_not_go", *decision.get("blockers", [])]
    return []
...
def monitor_checklist() -> list[str]:
    return [
        "Load the M94 execution evidence before choosing any next commercial movement.",
...
def monitor_decision_from(
    ...
    if source_result not in set(config.get("allowedSourceExecutionResults", [])):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_source_result_invalid")
    if monitor_status not in set(config.get("allowedMonitorStatuses", [])):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_status_invalid")
    if decision not in set(config.get("allowedDecisions", [])):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_decision_invalid")
    if execution and source_result != source_execution_result(execution):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_source_not_m94_result")
    if any(value < 0 for value in (observation_hours, responses, positive_signals, support_items_open, refund_requests, claims_issues, incidents)):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_metrics_invalid")
    if not safe_text(owner):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_owner_missing_or_unsafe")
    if not monitor_notes.strip():
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_notes_missing")
    if not next_action.strip():
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_next_action_missing")
    if observation_hours < int(config.get("minimumObservationHours", 24)):
        blockers.append("approved_controlled_commercial_movement_from_m93_execution_monitor_requires_observation_time")
    ...
    for name, (value, maximum) in risk_counts.items():
        if value > maximum:
            blockers.append(f"approved_controlled_commercial_movement_from_m93_execution_monitor_{name}_requires_hold_or_pause")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Approved Controlled Commercial Movement From M93 Execution Monitor Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"approved_controlled_commercial_movement_from_m93_execution_monitor_{current_stamp}.json"
    md_path = output_dir / f"approved_controlled_commercial_movement_from_m93_execution_monitor_{current_stamp}.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Monitor the M94 execution result before any additional commercial movement.")
