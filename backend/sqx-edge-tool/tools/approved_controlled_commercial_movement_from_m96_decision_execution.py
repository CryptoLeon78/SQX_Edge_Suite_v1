...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "approved_controlled_commercial_movement_from_m96_decision_execution.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "approved_controlled_commercial_movement_from_m96_decision_execution"
DEFAULT_DECISION_DIR = TOOL_ROOT / "data" / "next_controlled_commercial_movement_from_m95_decision"
EXPECTED_STATE = "approved_controlled_commercial_movement_from_m96_decision_execution_ready"
...
def latest_decision_file(directory: Path = DEFAULT_DECISION_DIR) -> Path | None:
    return latest_file(directory, "next_controlled_commercial_movement_from_m95_decision_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    decision_config_path = project_path(str(depends_on.get("nextControlledCommercialMovementFromM95DecisionConfig", "")))
    if not decision_config_path.is_file():
        return [*blockers, "next_controlled_commercial_movement_from_m95_decision_config_missing"]
    decision_config = load_json(decision_config_path)
    if decision_config.get("state") != depends_on.get("nextControlledCommercialMovementFromM95DecisionState"):
...
def validate_source(decision_record: dict[str, Any] | None, allow_no_go_decision: bool) -> list[str]:
    if decision_record is None:
        return ["next_controlled_commercial_movement_from_m95_decision_evidence_missing"]
    decision = decision_record.get("decision", {})
    if not decision.get("go") and not allow_no_go_decision:
        return ["next_controlled_commercial_movement_from_m95_decision_not_go", *decision.get("blockers", [])]
    return []
...
def execution_checklist() -> list[str]:
    return [
        "Load the approved M96 next-movement decision before recording execution.",
        "Record only the exact M96-approved manual movement.",
...
def execution_decision_from(
    ...
    if source_movement not in set(config.get("allowedSourceNextMovements", [])):
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_source_invalid")
    if execution_result not in set(config.get("allowedExecutionResults", [])):
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_result_invalid")
    if decision_record and source_movement != source_next_movement(decision_record):
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_source_not_m96_movement")
    if expected_result and execution_result != expected_result:
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_result_not_allowed_for_m96_movement")
    if any(value < 0 for value in (new_private_links, new_invites)):
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_metrics_invalid")
    if not safe_text(owner):
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_owner_missing_or_unsafe")
    if not execution_summary.strip():
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_summary_missing")
    if not next_monitor.strip():
        blockers.append("approved_controlled_commercial_movement_from_m96_decision_execution_next_monitor_missing")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Approved Controlled Commercial Movement From M96 Decision Execution Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"approved_controlled_commercial_movement_from_m96_decision_execution_{current_stamp}.json"
    md_path = output_dir / f"approved_controlled_commercial_movement_from_m96_decision_execution_{current_stamp}.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Record the exact manual execution approved by the M96 next-movement gate.")
