...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "controlled_traffic_expansion_execution_monitor.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "controlled_traffic_expansion_execution_monitor"
DEFAULT_EXECUTION_DIR = TOOL_ROOT / "data" / "controlled_traffic_expansion_execution"
EXPECTED_STATE = "controlled_traffic_expansion_execution_monitor_ready"
...
def latest_execution_file(directory: Path = DEFAULT_EXECUTION_DIR) -> Path | None:
    return latest_file(directory, "controlled_traffic_expansion_execution_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    execution_config_path = project_path(str(depends_on.get("controlledTrafficExpansionExecutionConfig", "")))
    if not execution_config_path.is_file():
        return ["controlled_traffic_expansion_execution_config_missing"]
    execution_config = load_json(execution_config_path)
    if execution_config.get("state") != depends_on.get("controlledTrafficExpansionExecutionState"):
        return ["controlled_traffic_expansion_execution_state_invalid"]
    return []


def validate_execution(execution: dict[str, Any] | None, allow_no_go_execution: bool) -> list[str]:
    if execution is None:
        return ["controlled_traffic_expansion_execution_evidence_missing"]
    decision = execution.get("decision", {})
    if not decision.get("go") and not allow_no_go_execution:
        return ["controlled_traffic_expansion_execution_not_go", *decision.get("blockers", [])]
    return []
...
def monitor_checklist() -> list[str]:
    return [
        "Load the M85 execution evidence before choosing any next movement.",
        "Record only redacted aggregate counts and operational signals.",
        "Block new decisions when support, refund, claims or incident risk appears.",
        "Keep the next movement gated by a new decision record.",
...
def monitor_decision_from(
    ...
    if source_result not in set(config.get("allowedSourceExecutionResults", [])):
        blockers.append("controlled_traffic_expansion_execution_monitor_source_result_invalid")
    if monitor_status not in set(config.get("allowedMonitorStatuses", [])):
        blockers.append("controlled_traffic_expansion_execution_monitor_status_invalid")
    if decision not in set(config.get("allowedDecisions", [])):
        blockers.append("controlled_traffic_expansion_execution_monitor_decision_invalid")
    if execution and source_result != source_execution_result(execution):
        blockers.append("controlled_traffic_expansion_execution_monitor_source_not_m85_result")
    if any(value < 0 for value in (observation_hours, responses, positive_signals, support_items_open, refund_requests, claims_issues, incidents)):
        blockers.append("controlled_traffic_expansion_execution_monitor_metrics_invalid")
    if not is_safe_text(owner):
        blockers.append("controlled_traffic_expansion_execution_monitor_owner_missing_or_unsafe")
    if not monitor_notes.strip():
        blockers.append("controlled_traffic_expansion_execution_monitor_notes_missing")
    if not next_action.strip():
        blockers.append("controlled_traffic_expansion_execution_monitor_next_action_missing")

    if observation_hours < int(config.get("minimumObservationHours", 24)):
        blockers.append("controlled_traffic_expansion_execution_monitor_requires_observation_time")
    ...
    for name, (value, maximum) in risk_counts.items():
        if value > maximum:
            blockers.append(f"controlled_traffic_expansion_execution_monitor_{name}_requires_hold_or_pause")

    if decision == "prepare_new_decision":
        if monitor_status not in {"completed", "partial"}:
            blockers.append("prepare_new_decision_requires_completed_or_partial_monitor")
        if positive_signals < int(config.get("minimumPositiveSignalsForNewDecision", 1)):
            blockers.append("prepare_new_decision_requires_positive_signal")
        if source_result in {"fix_hold_recorded", "sales_pause_recorded"}:
            blockers.append("prepare_new_decision_blocked_after_hold_or_pause")
        if risk_present:
            blockers.append("prepare_new_decision_blocked_by_risk")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Controlled Traffic Expansion Execution Monitor Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"controlled_traffic_expansion_execution_monitor_{current_stamp}.json"
    md_path = output_dir / f"controlled_traffic_expansion_execution_monitor_{current_stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    md_path.write_text(markdown_report(report), encoding="utf-8")
    return {"json": str(json_path), "markdown": str(md_path)}


def collect_controlled_traffic_expansion_execution_monitor(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Monitor the M85 execution result before any further movement.")
...
def main() -> int:
    ...
    report = collect_controlled_traffic_expansion_execution_monitor(
