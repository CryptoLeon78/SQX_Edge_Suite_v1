...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "template_pack_2_handoff.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "template_pack_2_handoff"
DEFAULT_PURCHASE_DRILL_DIR = TOOL_ROOT / "data" / "template_pack_2_purchase_drill"
...
def latest_purchase_drill_file(directory: Path = DEFAULT_PURCHASE_DRILL_DIR) -> Path | None:
    return latest_file(directory, "template_pack_2_purchase_drill_*.json")
...
def validate_purchase_drill(purchase_drill: dict[str, Any] | None, allow_no_go_purchase_drill: bool) -> list[str]:
    if purchase_drill is None:
        return ["template_pack_2_purchase_drill_evidence_missing"]
    decision = purchase_drill.get("decision", {})
    if not decision.get("go") and not allow_no_go_purchase_drill:
        return ["template_pack_2_purchase_drill_not_go", *decision.get("blockers", [])]
    return []
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Template Pack 2 Post-Purchase Handoff Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"template_pack_2_handoff_{current_stamp}.json"
    md_path = output_dir / f"template_pack_2_handoff_{current_stamp}.md"
...
def collect_handoff(
    ...
    report: dict[str, Any] = {
        "created_at": now_iso(),
        "state": config.get("state", "template_pack_2_handoff_ready"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Template Pack 2 post-purchase handoff and scale decision.")
    ...
    if args.use_latest_purchase_drill and purchase_drill_file is None:
        ...
        if purchase_drill_file is None:
            print(json.dumps({"ok": False, "error": "template_pack_2_purchase_drill_evidence_missing"}, indent=2, sort_keys=True))
            return 2
