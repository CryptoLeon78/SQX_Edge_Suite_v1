...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "next_controlled_commercial_movement_from_m95_decision.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "next_controlled_commercial_movement_from_m95_decision"
DEFAULT_MONITOR_DIR = TOOL_ROOT / "data" / "approved_controlled_commercial_movement_from_m93_execution_monitor"
EXPECTED_STATE = "next_controlled_commercial_movement_from_m95_decision_ready"
...
def latest_monitor_file(directory: Path = DEFAULT_MONITOR_DIR) -> Path | None:
    return latest_file(directory, "approved_controlled_commercial_movement_from_m93_execution_monitor_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    if config.get("state") != EXPECTED_STATE:
        blockers.append("next_controlled_commercial_movement_from_m95_decision_state_invalid")
    depends_on = config.get("dependsOn") if isinstance(config.get("dependsOn"), dict) else {}
    monitor_config_path = project_path(str(depends_on.get("approvedControlledCommercialMovementFromM93ExecutionMonitorConfig", "")))
    if not monitor_config_path.is_file():
        return [*blockers, "approved_controlled_commercial_movement_from_m93_execution_monitor_config_missing"]
    monitor_config = load_json(monitor_config_path)
    if monitor_config.get("state") != depends_on.get("approvedControlledCommercialMovementFromM93ExecutionMonitorState"):
...
def validate_monitor(monitor: dict[str, Any] | None, allow_no_go_monitor: bool) -> list[str]:
    if monitor is None:
        return ["approved_controlled_commercial_movement_from_m93_execution_monitor_evidence_missing"]
    decision = monitor.get("decision", {})
    if not decision.get("go") and not allow_no_go_monitor:
        return ["approved_controlled_commercial_movement_from_m93_execution_monitor_not_go", *decision.get("blockers", [])]
    return []
...
def decision_checklist() -> list[str]:
    return [
        "Load the M95 monitor evidence before deciding any next commercial movement.",
...
def decision_from(
    ...
    if source_decision not in set(config.get("allowedSourceMonitorDecisions", [])):
        blockers.append("next_controlled_commercial_movement_from_m95_source_decision_invalid")
    if next_movement not in set(config.get("allowedNextMovements", [])):
        blockers.append("next_controlled_commercial_movement_from_m95_invalid")
    if monitor and source_decision != source_monitor_decision:
        blockers.append("next_controlled_commercial_movement_from_m95_source_not_m95_decision")
    if any(value < 0 for value in (observation_hours, positive_signals, support_items_open, refund_requests, claims_issues, incidents)):
        blockers.append("next_controlled_commercial_movement_from_m95_metrics_invalid")
    if not safe_text(owner):
        blockers.append("next_controlled_commercial_movement_from_m95_owner_missing_or_unsafe")
    if not rationale.strip():
        blockers.append("next_controlled_commercial_movement_from_m95_rationale_missing")
    if not next_gate.strip():
        blockers.append("next_controlled_commercial_movement_from_m95_next_gate_missing")
    if observation_hours < int(config.get("minimumObservationHours", 24)):
        blockers.append("next_controlled_commercial_movement_from_m95_requires_observation_time")
    ...
    for name, (value, maximum) in risk_counts.items():
        if value > maximum:
            blockers.append(f"next_controlled_commercial_movement_from_m95_{name}_requires_hold_or_pause")

    if next_movement == "prepare_next_micro_step":
        if source_decision != "prepare_next_decision":
            blockers.append("prepare_next_micro_step_requires_m95_prepare_next_decision")
    ...
    if next_movement == "prepare_private_review_packet":
        if source_decision not in {"prepare_next_decision", "prepare_private_review_packet"}:
            blockers.append("prepare_private_review_packet_requires_m95_review_or_decision")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Next Controlled Commercial Movement From M95 Decision Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"next_controlled_commercial_movement_from_m95_decision_{current_stamp}.json"
    md_path = output_dir / f"next_controlled_commercial_movement_from_m95_decision_{current_stamp}.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide the next controlled commercial movement from M95 evidence.")
