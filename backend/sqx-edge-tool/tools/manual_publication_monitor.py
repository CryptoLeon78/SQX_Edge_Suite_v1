...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "manual_publication_monitor.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "manual_publication_monitor"
DEFAULT_RECORD_DIR = TOOL_ROOT / "data" / "manual_limited_publication_record"
EXPECTED_STATE = "manual_publication_monitor_ready"
...
def latest_record_file(directory: Path = DEFAULT_RECORD_DIR) -> Path | None:
    return latest_file(directory, "manual_limited_publication_record_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    record_config_path = project_path(str(depends_on.get("manualLimitedPublicationRecordConfig", "")))
    if not record_config_path.is_file():
        return ["manual_limited_publication_record_config_missing"]
    record_config = load_json(record_config_path)
    if record_config.get("state") != depends_on.get("manualLimitedPublicationRecordState"):
        return ["manual_limited_publication_record_state_invalid"]
    return []


def validate_record(record: dict[str, Any] | None, allow_no_go_record: bool) -> list[str]:
    if record is None:
        return ["manual_limited_publication_record_evidence_missing"]
    decision = record.get("decision", {})
    if not decision.get("go") and not allow_no_go_record:
        return ["manual_limited_publication_record_not_go", *decision.get("blockers", [])]
    if record.get("publication_decision") != "record_manual_publication" and not allow_no_go_record:
        return ["m79_did_not_select_record_manual_publication"]
    return []
...
def monitor_checklist() -> list[str]:
    return [
        "Load M79 manual publication record before monitoring.",
...
def decision_from(
    ...
    if decision not in set(config.get("allowedDecisions", [])):
        blockers.append("manual_publication_monitor_decision_invalid")
    if any(value < 0 for value in (observation_hours, views, clicks, support_gaps, refund_requests, claims_flags, incidents, positive_signals)):
        blockers.append("manual_publication_monitor_metrics_invalid")
    if not is_safe_text(owner):
        blockers.append("manual_publication_monitor_owner_missing_or_unsafe")
    if not monitor_notes.strip():
        blockers.append("manual_publication_monitor_notes_missing")
    if not next_action.strip():
        blockers.append("manual_publication_monitor_next_action_missing")
    if not support_ready:
        blockers.append("manual_publication_monitor_requires_support_ready")
    if not rollback_ready:
        blockers.append("manual_publication_monitor_requires_rollback_ready")
    if not pause_rule_ready:
        blockers.append("manual_publication_monitor_requires_pause_rule")
    ...
    for name, (value, maximum) in risky_counts.items():
        if value > maximum:
            blockers.append(f"manual_publication_monitor_{name}_require_hold_or_pause")

    if decision == "prepare_traffic_expansion_review":
        if observation_hours < int(config.get("minimumObservationHours", 24)):
            blockers.append("prepare_traffic_expansion_review_requires_observation_time")
        if positive_signals < int(config.get("minimumPositiveSignalsForExpansionReview", 1)):
            blockers.append("prepare_traffic_expansion_review_requires_positive_signal")
        if any(value > maximum for value, maximum in risky_counts.values()):
            blockers.append("risky_counts_block_traffic_expansion_review")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Manual Publication Monitor Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"manual_publication_monitor_{current_stamp}.json"
    md_path = output_dir / f"manual_publication_monitor_{current_stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    md_path.write_text(markdown_report(report), encoding="utf-8")
    return {"json": str(json_path), "markdown": str(md_path)}


def collect_manual_publication_monitor(
    ...
    return {
        ...
        "source_publication_decision": record.get("publication_decision", "") if record else "",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Monitor a manual limited publication before traffic expansion.")
...
def main() -> int:
    ...
    report = collect_manual_publication_monitor(
