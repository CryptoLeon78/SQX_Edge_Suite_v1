...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "template_pack_2_feedback_cohort.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "template_pack_2_feedback_cohort"
DEFAULT_SALES_REGISTER_DIR = TOOL_ROOT / "data" / "template_pack_2_sales_register"
...
def latest_sales_register_file(directory: Path = DEFAULT_SALES_REGISTER_DIR) -> Path | None:
    return latest_file(directory, "template_pack_2_sales_register_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    if not sales_config_path.is_file():
        return ["template_pack_2_sales_register_config_missing"]
    sales_config = load_json(sales_config_path)
    if sales_config.get("state") != depends_on.get("salesRegisterState"):
        return ["template_pack_2_sales_register_state_invalid"]
    return []
...
def validate_sales_register(sales_register: dict[str, Any] | None, allow_no_go_sales_register: bool) -> list[str]:
    if sales_register is None:
        return ["template_pack_2_sales_register_evidence_missing"]
    decision = sales_register.get("decision", {})
    if not decision.get("go") and not allow_no_go_sales_register:
        return ["template_pack_2_sales_register_not_go", *decision.get("blockers", [])]
    return []
...
def feedback_checklist() -> list[str]:
    return [
        ...
        "Decide expand_traffic, iterate_offer, prepare_pack_3 or pause_sales.",
    ]
...
def decision_from(
    ...
    min_positive_pack3 = int(config.get("minimumPositiveSignalsForPack3", 2))
    ...
    if open_support_items > 0 and roadmap_decision in {"expand_traffic", "prepare_pack_3"}:
    ...
    if roadmap_decision == "prepare_pack_3":
        if positive_signals < min_positive_pack3:
            blockers.append("pack_3_needs_positive_signals")
        if blocking_bugs > 0 or activation_friction > 0 or refund_count > 0:
            blockers.append("pack_3_blocked_by_risk")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Template Pack 2 Feedback Cohort Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"template_pack_2_feedback_cohort_{current_stamp}.json"
    md_path = output_dir / f"template_pack_2_feedback_cohort_{current_stamp}.md"
...
def collect_feedback_cohort(
    ...
    report: dict[str, Any] = {
        "created_at": now_iso(),
        "state": config.get("state", "template_pack_2_feedback_cohort_ready"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Review Template Pack 2 add-on buyer feedback cohort before scaling traffic.")
    ...
    if args.use_latest_sales_register and sales_register_file is None:
        ...
        if sales_register_file is None:
            print(json.dumps({"ok": False, "error": "template_pack_2_sales_register_evidence_missing"}, indent=2, sort_keys=True))
            return 2
