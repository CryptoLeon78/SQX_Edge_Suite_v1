...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "template_pack_1_sales_register.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "template_pack_1_sales_register"
DEFAULT_HANDOFF_DIR = TOOL_ROOT / "data" / "template_pack_1_handoff"
...
def latest_handoff_file(directory: Path = DEFAULT_HANDOFF_DIR) -> Path | None:
    return latest_file(directory, "template_pack_1_handoff_*.json")
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Template Pack 1 Add-On Sales Register Evidence",
        "",
...
def write_evidence(report: dict[str, Any], output_dir: Path, append_register: bool) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"template_pack_1_sales_register_{current_stamp}.json"
    md_path = output_dir / f"template_pack_1_sales_register_{current_stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    md_path.write_text(markdown_report(report), encoding="utf-8")
    paths = {"json": str(json_path), "markdown": str(md_path)}
    if append_register:
        register_path = output_dir / "template_pack_1_sales_register.jsonl"
...
def collect_sales_register(
    ...
    report: dict[str, Any] = {
        "created_at": now_iso(),
        "state": config.get("state", "template_pack_1_sales_register_ready"),
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Template Pack 1 add-on sales register before scaling traffic.")
    ...
    if args.use_latest_handoff and handoff_file is None:
        ...
        if handoff_file is None:
            print(json.dumps({"ok": False, "error": "template_pack_1_handoff_evidence_missing"}, indent=2, sort_keys=True))
            return 2
