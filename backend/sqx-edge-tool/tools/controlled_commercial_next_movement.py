...
DEFAULT_CONFIG = TOOL_ROOT / "config" / "controlled_commercial_next_movement.json"
DEFAULT_MANIFEST = TOOL_ROOT / "config" / "product_manifest.json"
DEFAULT_OUTPUT = TOOL_ROOT / "data" / "controlled_commercial_next_movement"
DEFAULT_MONITOR_DIR = TOOL_ROOT / "data" / "controlled_traffic_expansion_execution_monitor"
EXPECTED_STATE = "controlled_commercial_next_movement_ready"
...
def latest_monitor_file(directory: Path = DEFAULT_MONITOR_DIR) -> Path | None:
    return latest_file(directory, "controlled_traffic_expansion_execution_monitor_*.json")
...
def validate_dependency(config: dict[str, Any]) -> list[str]:
    ...
    monitor_config_path = project_path(str(depends_on.get("controlledTrafficExpansionExecutionMonitorConfig", "")))
    if not monitor_config_path.is_file():
        return ["controlled_traffic_expansion_execution_monitor_config_missing"]
    monitor_config = load_json(monitor_config_path)
    if monitor_config.get("state") != depends_on.get("controlledTrafficExpansionExecutionMonitorState"):
        return ["controlled_traffic_expansion_execution_monitor_state_invalid"]
    return []


def validate_monitor(monitor: dict[str, Any] | None, allow_no_go_monitor: bool) -> list[str]:
    if monitor is None:
        return ["controlled_traffic_expansion_execution_monitor_evidence_missing"]
    decision = monitor.get("decision", {})
    if not decision.get("go") and not allow_no_go_monitor:
        return ["controlled_traffic_expansion_execution_monitor_not_go", *decision.get("blockers", [])]
    return []
...
def next_movement_checklist() -> list[str]:
    return [
        "Load the M86 monitor evidence before deciding any next movement.",
...
def decision_from(
    ...
    if source_decision not in set(config.get("allowedSourceMonitorDecisions", [])):
        blockers.append("controlled_commercial_next_movement_source_decision_invalid")
    if next_movement not in set(config.get("allowedNextMovements", [])):
        blockers.append("controlled_commercial_next_movement_invalid")
    if monitor and source_decision != source_monitor_decision:
        blockers.append("controlled_commercial_next_movement_source_not_m86_decision")
    if any(value < 0 for value in (observation_hours, positive_signals, support_items_open, refund_requests, claims_issues, incidents)):
        blockers.append("controlled_commercial_next_movement_metrics_invalid")
    if not is_safe_text(owner):
        blockers.append("controlled_commercial_next_movement_owner_missing_or_unsafe")
    if not rationale.strip():
        blockers.append("controlled_commercial_next_movement_rationale_missing")
    if not next_gate.strip():
        blockers.append("controlled_commercial_next_movement_next_gate_missing")

    if observation_hours < int(config.get("minimumObservationHours", 24)):
        blockers.append("controlled_commercial_next_movement_requires_observation_time")
    ...
    for name, (value, maximum) in risk_counts.items():
        if value > maximum:
            blockers.append(f"controlled_commercial_next_movement_{name}_requires_hold_or_pause")

    if next_movement == "prepare_next_micro_step":
        if source_decision != "prepare_new_decision":
            blockers.append("prepare_next_micro_step_requires_m86_prepare_new_decision")
    ...
    if next_movement == "prepare_private_review_packet":
        if source_decision != "prepare_new_decision":
            blockers.append("prepare_private_review_packet_requires_m86_prepare_new_decision")
        if source_monitor_status != "completed":
...
def markdown_report(report: dict[str, Any]) -> str:
    ...
    lines = [
        "# Controlled Commercial Next Movement Evidence",
        "",
    ...
    lines.extend(f"- {item}" for item in report["next_movement_checklist"])
    lines.append("")
...
def write_evidence(report: dict[str, Any], output_dir: Path) -> dict[str, str]:
    ...
    current_stamp = stamp()
    json_path = output_dir / f"controlled_commercial_next_movement_{current_stamp}.json"
    md_path = output_dir / f"controlled_commercial_next_movement_{current_stamp}.md"
    json_path.write_text(json.dumps(report, indent=2, sort_keys=True), encoding="utf-8")
    md_path.write_text(markdown_report(report), encoding="utf-8")
    return {"json": str(json_path), "markdown": str(md_path)}


def collect_controlled_commercial_next_movement(
    ...
    return {
        ...
        "next_gate": next_gate,
        "confirmations": confirmations,
        "next_movement_checklist": next_movement_checklist(),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Decide the next controlled commercial movement from M86 evidence.")
...
def main() -> int:
    ...
    report = collect_controlled_commercial_next_movement(
