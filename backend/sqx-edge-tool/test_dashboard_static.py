...
class DashboardStaticTestCase(unittest.TestCase):
    ...
    def test_t10ah_next_proxy_migration_is_documented_and_safe(self):
        ...
        for pattern in (
            ...
            "Next implementation phase: T10ai - prepare the Cloudflare provider-project preflight without deployment or tester URL. Historical anchor only; superseded by",
        ):
            with self.subTest(pattern=pattern):
                self.assertIn(pattern, governance)
