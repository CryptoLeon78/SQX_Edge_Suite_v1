...
class DashboardStaticTestCase(unittest.TestCase):
    ...
    def test_project_generator_onboarding_delegates_to_module(self):
        ...
        self.assertIn("function pgReadConfigInputs()", main_js)
        self.assertIn("function pgWriteConfigInputs(config)", main_js)
        self.assertIn("function pgApplySqxCandidate(candidate, outputEl)", main_js)
        self.assertIn("function pgApplyValidatedSqxPath(path, response)", main_js)
        self.assertIn("function pgUpdateMiningSummary(count)", main_js)
        self.assertIn("function pgRenderMiningsList(infos)", main_js)
        self.assertIn("function pgRenderOutputState(output)", main_js)
        ...
        self.assertNotIn("document.getElementById('pg-sqx-path').value = c.sqx_path", main_js)
        self.assertNotIn("document.getElementById('pg-sqx-path').value.trim()", main_js)
        self.assertNotIn("document.getElementById('pg-minings-count').textContent", main_js)
        self.assertNotIn("document.getElementById('pg-output-count').textContent", main_js)
        self.assertNotIn("document.getElementById('pg-minings-table').innerHTML", main_js)
        self.assertNotIn("document.getElementById('pg-output-list').innerHTML", main_js)
        ...
        self.assertIn("addEventListener('click', pgOpenOutputFolder)", main_js)
        self.assertNotIn("document.getElementById('pg-open-output').addEventListener('click', async function", main_js)
        self.assertNotIn("pgLog('📁 Abierta carpeta output'", main_js)
