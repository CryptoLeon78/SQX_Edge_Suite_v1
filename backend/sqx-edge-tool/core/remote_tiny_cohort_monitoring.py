...
DEFAULT_REMOTE8F_EVIDENCE_PATH = (
    PROJECT_ROOT / ".local" / "remote_service" / "remote8f_tiny_cohort_monitoring.local.json"
)
DEFAULT_REMOTE8F_OUTPUT_ROOT = PROJECT_ROOT / ".local" / "remote_service" / "remote8f_tiny_cohort_monitoring"
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("remote8f_monitoring_must_be_json_object")
    return payload
...
def _allowed_next_action(requested_decision: str, valid: bool) -> str:
    if not valid:
        return "fix_remote8f_blockers"
    if requested_decision == "prepare_next_controlled_movement":
        return "prepare_remote8g_tiny_cohort_decision_review"
    if requested_decision == "stay_tiny_cohort":
        return "continue_tiny_cohort_monitoring"
    return "operator_review_required"


def build_remote8f_monitoring_summary(payload: Mapping[str, Any]) -> dict[str, Any]:
    """Build a public-safe REMOTE-8F tiny cohort monitoring summary."""
    ...
    source_status = str(source_gate.get("remote8eStatus") or "").strip()
    ...
    if schema_version != REMOTE_TINY_COHORT_MONITORING_VERSION:
        blockers.append("remote8f_schema_version_mismatch")
    ...
    if source_status != REQUIRED_SOURCE_STATUS:
        blockers.append("remote8e_execution_go_status_missing")
    ...
    if requested_decision in {"fix_blockers", "rollback_tiny_cohort"} and not (missing_signals or metric_blockers):
    ...
    summary: dict[str, Any] = {
        ...
        "status": (
            ...
            if valid
            else "NO_GO_REMOTE8F_TINY_COHORT_MONITORING_BLOCKED"
        ),
        "version": REMOTE_TINY_COHORT_MONITORING_VERSION,
        ...
        "monitoring": {
            ...
            "cohortUserCount": user_count,
            "users": _public_users(users),
        },
        "sourceGate": {
            "remote8eStatus": source_status,
            "remote8eExecutionId": _short(source_gate.get("remote8eExecutionId")),
        },
        "signals": signals,
    ...
    if leaks:
        summary["ok"] = False
        summary["status"] = "NO_GO_REMOTE8F_PUBLIC_SUMMARY_PRIVACY_LEAK"
        summary["blockers"] = [*summary["blockers"], *leaks]
        summary["decision"]["ready"] = False
        summary["decision"]["allowedNextAction"] = "fix_remote8f_privacy_leak"
        summary["safety"]["monitoringWindowClean"] = False
    return summary


def ingest_remote8f_tiny_cohort_monitoring(
    *,
    evidence_path: str | Path | None = None,
    output_root: str | Path | None = None,
) -> dict[str, Any]:
    """Validate ignored REMOTE-8F monitoring evidence and write a redacted summary."""
    source = Path(evidence_path).expanduser().resolve(strict=False) if evidence_path else DEFAULT_REMOTE8F_EVIDENCE_PATH
    output_base = Path(output_root).expanduser().resolve(strict=False) if output_root else DEFAULT_REMOTE8F_OUTPUT_ROOT
    summary_path = output_base / "remote8f_tiny_cohort_monitoring.public.json"

    if not source.is_file():
        summary = {
            "ok": False,
            "status": "NO_GO_REMOTE8F_MONITORING_EVIDENCE_MISSING",
            "version": REMOTE_TINY_COHORT_MONITORING_VERSION,
            "generatedAt": _utc_now(),
            "error": "remote8f_monitoring_evidence_missing",
            "missingSignals": list(REQUIRED_SIGNALS),
            "blockers": ["tiny_cohort_monitoring_evidence_missing"],
            "decision": {
                "ready": False,
                "allowedNextAction": "collect_remote8f_private_evidence",
                "automationAllowed": False,
                "furtherExpansionAllowedNow": False,
                "requiresOperatorApprovalForNextMovement": True,
            },
            ...
            "nextPhase": "REMOTE-8F-private-monitoring-evidence-required",
        }
        _write_json(summary_path, summary)
        return summary

    payload = _load_json(source)
    summary = build_remote8f_monitoring_summary(payload)
    _write_json(summary_path, summary)
    return summary
