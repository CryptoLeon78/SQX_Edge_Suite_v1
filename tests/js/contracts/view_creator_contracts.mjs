...
assert.equal(viewCreator.metrics.length, 96, 'catalog should preserve the imported metric universe plus SQX Edge correlation columns');
...
assert.ok(buyerTemplates.some(template => template.id === 'sqx-edge-correlation-review' && template.name === 'SQX EDGE CORRELATION REVIEW' && template.priority === 'obligatoria'));
...
assert.ok(buyerTemplates.some(template => template.id === 'sqx-edge-correlation-review' && template.oosTag === 'corr1' && template.config.yearCount === 1));
...
const tmCert = buyerTemplates.find(template => template.id === 'sqx-edge-correlation-review');
assert.ok(tmCert.config.metrics.some(metric => metric.className === 'SQXEdgeCorrDecision'));
assert.ok(tmCert.config.metrics.some(metric => metric.className === 'ReturnDDRatio'));
assert.ok(tmCert.metricTags.includes('CORR1'));
assert.ok(tmCert.metricTags.includes('Decision'));
const templateMakerCertXml = viewCreator.buildTemplateMakerCertView();
assert.ok(templateMakerCertXml.includes('name="SQX EDGE CORRELATION REVIEW"'));
assert.equal((templateMakerCertXml.match(/<Column /g) || []).length, 17, 'Correlation review helper should match the active CORR1 C2 contract');
assert.ok(templateMakerCertXml.includes('class="SQXEdgeCorrDecision" name="SQX Edge Corr Decision"'));
assert.ok(templateMakerCertXml.includes('class="ReturnDDRatio" name="Ret/DD Ratio"'));
...
assert.ok(buyerPack.presets.some(preset => preset.id === 'buyer-sqx-edge-correlation-review'));
