...
assert.ok(buyerTemplates.some(template => template.id === 'template-maker-cert' && template.oosTag === '2oos' && template.config.yearCount === 2));
...
assert.ok(tmCert.metricTags.includes('Forward CSV'));
...
assert.equal((templateMakerCertXml.match(/<Column /g) || []).length, 42, 'Template Maker Cert helper should match the active 2-OOS Forward contract');
assert.ok(templateMakerCertXml.includes('class="NetProfit" name="Net profit" sampleType="127"'));
assert.ok(templateMakerCertXml.includes('class="WinningPct" name="Winning Percent" sampleType="22"'));
