...
assert.match(tool, /sqx142-portfolio-corr2-local-custom-project-integration-v1/);
...
assert.match(edgeFactoryUi, /\/sqx142\/portfolio-corr2\/local-project/);
...
assert.match(html, /Parchear custom SQX/);
assert.match(css, /edge-corr2-controls/);

assert.match(doc, /Status: `implemented_local_project_patch_ready`/);
