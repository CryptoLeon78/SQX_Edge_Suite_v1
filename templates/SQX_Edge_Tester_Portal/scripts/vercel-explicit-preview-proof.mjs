...
const intendedPreviewBranch = process.env.T10C_PREVIEW_BRANCH || "tester-preview";
const expectedProductionBranch = process.env.T10C_EXPECTED_PRODUCTION_BRANCH || "main";
...
if (!existsSync(projectPath)) {
  fail("NO_GO_PROJECT_NOT_LINKED", {
    reason: ".vercel/project.json is required locally to prove the explicit API preview path",
...
if (reservedBranches.has(intendedPreviewBranch.toLowerCase())) {
  fail("NO_GO_PREVIEW_BRANCH_RESERVED", {
    projectName: localProject.projectName,
    intendedPreviewBranch,
    reason: "The explicit preview branch must not be production-like",
...
if (!git.linked) {
  fail("NO_GO_GIT_SOURCE_NOT_LINKED", {
    ...
    git,
    reason: "The project must stay linked to the private Git repository before an explicit preview deploy can be built",
...
console.log(JSON.stringify({
  ok: true,
  phase: "T10c",
  status: "GO_EXPLICIT_API_PREVIEW_PATH_READY",
  project: summarizeProject(project),
  git,
  expectedProductionBranch,
  ...
  externalDeployAllowed: false,
  externalDeployAttempted: false,
  requiredNextAction: "T10d may execute an explicit API preview deployment, then inspect target=preview before any URL is shared"
}, null, 2));
