...
$Version = 'sqx142-ai-wizard-overlay-v2'
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$SourceRoot = Join-Path $RepoRoot 'integrations\sqx142\ai_wizard_overlay'
$CssName = 'sqx-edge-aiwizard.css'
$JsName = 'sqx-edge-aiwizard.js'
$Marker = 'sqx-edge-aiwizard'
...
function New-Backup {
  ...
  $backup = Join-Path $BackupRoot "sqx142_ai_wizard_overlay_$stamp"
...
function Install-Overlay {
  ...
  if ($blockers.Count -gt 0) {
    return New-Result -Ok $false -Status 'blocked' -Blockers $blockers
  }
  if (-not $Apply) {
    return New-Result -Ok $true -Status 'ready_to_install' -Extra @{ installed = $state.includesPresent }
  }
  ...
  $html = [regex]::Replace($html, '(?im)^[^\r\n]*(sqx-edge-aiwizard\.css|sqx-edge-aiwizard\.js)(\?v=[^"`'']+)?[^\r\n]*(\r?\n)?', '')
  ...
  return New-Result -Ok $true -Status 'installed' -Extra @{ backupRef = (Split-Path $backup -Leaf); installed = $true }
}
...
function Rollback-Overlay {
  ...
  $baseline = Get-ChildItem -LiteralPath $BackupRoot -Directory -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -like 'sqx142_ai_wizard_overlay_*' } |
