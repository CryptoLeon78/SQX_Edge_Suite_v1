...
- Capa1 Fastest Precision Correction Before Capa2 SPP is the active precision policy for Capa1 robustness: Capa1 robustness retests MC through WFM use `testPrecision=1 fastest`, while Capa1 Foward remains `testPrecision=2` tick precision and TICK REAL audited/inactive setups stay `testPrecision=2`. This correction supersedes older Capa1 closeout text and did not start Capa2 SPP.
