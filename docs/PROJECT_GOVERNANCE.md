...
- Current phase completed: REMOTE-8 - controlled pilot drill for signed entitlement, app login, workspace, `.cfx` artifact, export, isolation, revocation and restore.
- Current implementation phase: REMOTE-8B - private live pilot evidence ingest for one real user, with redacted evidence only and no cohort expansion without explicit approval.
