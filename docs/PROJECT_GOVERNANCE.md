...
- Current phase completed: REMOTE-8H cycle bridge - package source now accepts REMOTE-8L decisions while preserving legacy REMOTE-8G compatibility.
- Current implementation phase: REMOTE-8H private package evidence - package exactly one next movement only if REMOTE-8L selects `prepare_next_controlled_movement`.
