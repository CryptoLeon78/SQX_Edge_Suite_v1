...
- Current implementation phase: REMOTE-0 - document the strategic pivot from portable/local distribution to `remote_service`: paid validated-email access through a protected link, laptop-hosted pilot via Cloudflare Tunnel, mandatory per-user workspaces and portable ZIP retained only as internal fallback.
...
| Monetization/Product | Web Pro offer, pricing, support window, subscription access, buyer journey, safe claims. | `docs/REMOTE_SERVICE_ROADMAP.md`, `docs/MONETIZATION_ROADMAP.md`, `docs/MONETIZATION_M*.md`, `docs/sales`, product manifest, README. | Commercial docs tests, safe-claims review, roadmap/status alignment, no portable-first buyer copy. |
| Security/Distribution | Secrets, payment/webhook material, tunnel exposure, workspace isolation, fallback packaging exclusions. | `.gitignore`, product manifest security, Cloudflare/Tunnel docs, packaging/audit scripts, relay settings, license manager. | Sensitive-file staged review, tunnel exposure checks, workspace isolation tests, packaging audit rules. |
...
| Access/Security Gatekeeper | Paid remote auth, sessions, webhooks, Cloudflare Access/Tunnel, workspace access, audit, watermark and anti-abuse controls. | `docs/REMOTE_SERVICE_ROADMAP.md`, `docs/REMOTE_SERVICE_SECURITY_PRIVACY_COPY.md`, tester portal contracts, Cloudflare evidence, auth/session/payment docs. | Anonymous blocked, inactive payment blocked, active paid user allowed, workspace isolation proven, webhook signatures verified, rate limit and audit contracts, no secrets in git. |
