...
- Current phase completed: REMOTE-RUNBOOK1B - Root launcher cleanup. Root launchers are now remote-only (`START_SQX_EDGE_REMOTE.bat` / `STOP_SQX_EDGE_REMOTE.bat`), the visual monitor opens visibly, and legacy portable launchers remain only under `packaging/` or generated ZIPs.
...
- Recent remote anchor: REMOTE-ACCEPT1 - Real Browser Acceptance Gate. The Welcome copy hides internal app-session wording, `Acceso DASHBOARD` is the single user action after identity validation, and the next real check is private browser evidence before expanding testers.
- Current implementation phase: REMOTE-8C observation continues; next recommended value phase remains WAIT-3 Trust Evidence Pack only after the real browser acceptance path is clean. REMOTE-8C remains intentionally `NO_GO_REMOTE8C_FIRST_USER_OBSERVATION_BLOCKED` until the 24-hour window, guided flow, support loop and stability signals are clean.
- WAIT workstream state: WAIT-1 and WAIT-2 are completed. 2 of 5 WAIT phases are complete; no WAIT work may expand testers, grants, checkout or permissions without a separate approved phase.
