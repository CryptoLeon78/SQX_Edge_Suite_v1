...
`LOCAL_GBRAIN1` records a local searchable memory layer while Mem/gbrain write quota is unavailable.

- Marker: `sqx-edge-local-gbrain-v1`.
- Status: `local_gbrain_primary_mem_optional_quota_safe`.
- Local ignored storage is `.local/gbrain/local_gbrain.sqlite`.
- Tooling is `backend/sqx-edge-tool/core/local_gbrain.py` and `tools/local_gbrain.ps1 status|index|import-outbox|search|query|get-page|save-page`.
- Mode is `local_first_mem_optional`; import-outbox does not mark Mem notes synced.
