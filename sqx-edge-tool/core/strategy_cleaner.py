...
def backup_sqx(sqx_path: str) -> str:
    """Crea una copia de seguridad junto al .sqx antes de modificarlo."""
    if not os.path.isfile(sqx_path):
        raise FileNotFoundError(sqx_path)
    backup_dir = os.path.join(os.path.dirname(sqx_path), "__sqx_backups")
    os.makedirs(backup_dir, exist_ok=True)
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base = os.path.splitext(os.path.basename(sqx_path))[0]
    backup_path = os.path.join(backup_dir, f"{base}.{stamp}.sqx")
    counter = 2
    while os.path.exists(backup_path):
        backup_path = os.path.join(backup_dir, f"{base}.{stamp}.{counter}.sqx")
        counter += 1
    shutil.copy2(sqx_path, backup_path)
    return backup_path
...
def _process_one(args: tuple) -> dict:
    ...
    try:
        will_mutate = bool(opts.get("remove_exit_bars") or opts.get("rename_institutional"))
        if will_mutate and opts.get("backup", True):
            result["backup_path"] = backup_sqx(sqx_path)
            result["actions"].append(f"backup -> {os.path.basename(result['backup_path'])}")
