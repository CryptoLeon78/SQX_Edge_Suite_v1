...
try:
    import numpy as np
    import pandas as pd
except ImportError as exc:  # optional analysis dependencies
    np = None
    pd = None
...
from sqx_analysis_common import editorial_asset_map
...
def extract_dashboard_ratings() -> dict[str, dict]:
    """Lee los ratings editoriales desde el manifiesto actual del dashboard.
    Devuelve { asset_id: { 'subtype': str, 'type': str, 'ratings': { cat_key: rating } } }
    """
    return editorial_asset_map(ROOT)
...
def main(tf: str = "H1", out_path: Path | None = None) -> None:
    ...
    if ANALYSIS_IMPORT_ERROR is not None:
        raise SystemExit(
            "Dependencias analiticas no instaladas. Ejecuta: "
