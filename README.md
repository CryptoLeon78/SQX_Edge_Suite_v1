...
- Estado local: SQX142-AW-AI4 RSI Mean-Reversion Compiler queda construido e instalado como `sqx142-aw-ai4-rsi-mean-reversion-compiler-v1`. AI3 Catalog Expansion queda cerrado por roundtrip OK reportado por el operador; AI4 anade la primera familia nueva del catalogo como compilador verificable: `rsi_mean_reversion`. RSI puro o mean-reversion genera `.sqx` editable con `RSI(14) < 30` para long, `RSI(14) > 70` para short, direccion `long_only`/`short_only`/`both`, SL/TP desde prompt o defaults con `manualReviewRequired=true`; prompts multi-familia como RSI+Bollinger quedan bloqueados con `blocked_multi_family_compiler_not_ready`. Overlay instalado con backup `sqx142_ai_wizard_overlay_20260604_210522`; HTTP probe local confirma RSI draft OK y Keltner `blocked_not_draftable_yet`. Keltner/Bollinger/ADX/Stochastic siguen plan-only; sin SQX runtime launch, sin `data.db`, sin `user/projects` y sin databanks.
