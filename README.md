...
- Estado local: SQX142-AW-AI2 install/manual roundtrip fue intentado el 2026-06-04 y queda `blocked_sqx_process_running`. El status del overlay detecta assets presentes, pero la inspeccion read-only confirma que la version v2 no esta instalada y los assets activos no coinciden con la fuente repo; no se ejecuto `install -Apply`, no se forzo cierre de SQX, no hubo SQX runtime launch desde scripts, no `data.db`, no `user/projects`, no databanks, no licencia/activacion ni bypass. Siguiente paso seguro: cerrar SQX manualmente y reintentar `tools/sqx142_ai_wizard_overlay.ps1 install -Apply`, despues hacer el roundtrip manual en AlgoWizard.
