...
- Estado interno: T10e intento preview API con `target` omitido y la elimino al detectar que Vercel volvio a devolver `target=production`; no hay deployment activo ni URL compartida.
...
- T10e anade `proof:vercel-omitted-target-preview`, ejecuta una preview API con `target` omitido, detecta que Vercel vuelve a devolver `target=production`, elimina el deployment y deja T10f como recreacion/separacion obligatoria.
