...
- Estado interno: SQX142-PORTFOLIO-CORR1 Correlation Stability Audit esta implementado con decision registrada; `docs/SQX142_PORTFOLIO_CORR1_STABILITY_AUDIT.md` registra `sqx142-portfolio-corr1-stability-audit-v1`, `sqx142-portfolio-corr1-registered-decision-v1`, `POST /api/sqx142/portfolio-correlation/stability-audit`, `POST /api/sqx142/portfolio-corr1/registered-decision` y el panel Edge Factory `Auditoria de descorrelacion IS vs OOS3`. En el custom AUDCAD H1 real, `SQX EDGE CORR1 TAGGED=23` queda registrado en el paso 93 como `portfolio_decision`: `selectedByIs=1`, `similarByIs=22`, `review=0`; OOS3 audita y confirma que los 22 similares no son sustitutos. La regla queda fijada: `IS_CORR` selecciona/diversifica, `OOS3_CORR` solo audita estabilidad o veta; si OOS3/Forward se usa para elegir alternativos, se exige otro holdout posterior. Sin lanzar SQX, ejecutar retests, escribir `data.db`, mutar `user/projects`/databanks, parchear jars/plugins internos/licencia, usar `run_project`, Migration Tool ni `checkResources`.
- Estado interno: SQX142-PORTFOLIO-CORR2 Local Custom Project Integration esta aplicado; `docs/SQX142_PORTFOLIO_CORR2_LOCAL_CUSTOM_PROJECT_INTEGRATION.md` y `tools\sqx142_portfolio_corr2_local_project_integration.ps1` registran el integrador `status/plan/apply/record/rollback` para parchear directamente el custom SQX142 local con `Retest-Task4.xml` `CORR1 STABILITY RETEST` y `Retest-Task5.xml` `CORR1 TAG REVIEW`; el contrato activo usa tareas CORR1 activas, `Forward` como input y outputs `SQX EDGE CORR1 STABILITY` / `SQX EDGE CORR1 TAGGED`, `testPrecision=4`, backup/hash/rollback y registry steps. Edge Factory y Mining Control muestran botones `Estado CORR1 local`, `Registrar CORR1`, `Preflight CORR1`, `Parchear custom SQX` y `Rollback CORR1`. Se permite solo mutar el `project.cfx` del custom objetivo con SQX cerrado; siguen bloqueados jars, engine, licencia, `data.db`, borrado de databanks, `run_project`, Migration Tool y `checkResources`.
